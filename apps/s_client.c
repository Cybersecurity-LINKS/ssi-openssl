...
# endif
 ...
 opthelp:
    ...
    if(vc_file) {
    	...
    	fseek(vc, 0, SEEK_END);
    	long f_size = ftell(vc);
    	fseek(vc, 0, SEEK_SET);
    	vc_stream = malloc(f_size);

    	while ((c = fgetc(vc)) != EOF) {
    		vc_stream[n++] = (char) c;
    	}

    	vc_stream[n] = '\0';

    	SSL_CTX_set_vc(ctx, vc_stream);
    }
	...
	if (vc_file) {
		...
		if (!SSL_CTX_set_vc(ctx, vc_stream)) {
	...
	if(vc_issuers_file) {
		...
		vc_issuers = fopen(vc_issuers_file, "r");
